...
<div class="admin-grid-2" style="margin-bottom: 1.5rem;">
    ...
    <div class="admin-card">
        ...
        <div style="padding: 1.5rem;">
            <p class="admin-muted" style="margin-bottom: 1rem; font-size: 0.9rem;">
                Directly notify Google when pages are updated or removed. Requires a Google Cloud service account.
            </p>
...
<div class="admin-card" style="margin-bottom: 1.5rem;">
    ...
    <div style="padding: 1.5rem;">
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; text-align: center;">
            <div>
                <div style="font-size: 1.5rem; font-weight: 700;"><?= number_format($queueStats['pending']); ?></div>
                <div class="admin-muted" style="font-size: 0.8rem;">Pending</div>
            </div>
            <div>
                <div style="font-size: 1.5rem; font-weight: 700;"><?= number_format($queueStats['submitted']); ?></div>
                <div class="admin-muted" style="font-size: 0.8rem;">Submitted</div>
            </div>
            <div>
                <div style="font-size: 1.5rem; font-weight: 700;"><?= number_format($queueStats['error']); ?></div>
                <div class="admin-muted" style="font-size: 0.8rem;">Errors</div>
            </div>
        </div>
        <p class="admin-muted" style="margin-top: 1rem; font-size: 0.85rem;">
            Cron: <code class="idx-code" style="display: inline; padding: 0.125rem 0.375rem;">0 7 * * * php <?= htmlspecialchars(dirname(dirname(__DIR__))); ?>/cron/submit-indexing.php</code>
        </p>
    </div>
</div>
...
<div class="admin-card" style="margin-bottom: 1.5rem;">
    ...
    <?php if (empty($recentLogs)): ?>
        <div style="padding: 2rem; text-align: center; color: var(--admin-text-muted, var(--color-text-muted));">
...
.idx-code {
    ...
    border-radius: var(--admin-radius-sm, var(--radius-sm));
...
.idx-status-off {
    ...
    color: var(--admin-text-muted, var(--color-text-muted));
}
...
.idx-setup-steps a { color: var(--admin-primary, var(--color-purple)); text-decoration: none; }
.idx-setup-steps a:hover { text-decoration: underline; }

/* Two-column grid fallback if admin-grid-2 not defined */
.admin-grid-2 {
    display: grid;
