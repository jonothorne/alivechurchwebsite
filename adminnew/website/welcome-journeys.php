...
<div class="admin-dashboard-header" style="margin-bottom: 1rem;">
    ...
    <div>
        <a href="/adminnew/welcome-journey-preview" class="admin-btn admin-btn-secondary">Preview Emails</a>
    </div>
</div>
...
<div class="admin-card">
    <div class="admin-card-header">
        ...
        <a href="/adminnew/welcome-journeys?filter=<?= $filter; ?>" class="admin-btn admin-btn-sm admin-btn-secondary">Back</a>
    </div>
    ...
    <div style="margin-top: 1rem; padding: 1rem; background: var(--admin-bg); border: 1px solid var(--admin-border); border-radius: 8px;">
        <strong style="color: var(--admin-text);">Notes:</strong>
        <p style="margin: 0.5rem 0 0; color: var(--admin-text-muted);"><?= nl2br(htmlspecialchars($detail_journey['notes'])); ?></p>
    </div>
    <?php endif; ?>
    ...
    <div class="admin-timeline">
        ...
        <div class="admin-timeline-item">
            ...
            <div class="admin-timeline-content">
                ...
                <a href="?action=resend&email_id=<?= $email['id']; ?>&view=<?= $detail_journey['id']; ?>&filter=<?= $filter; ?>" class="admin-btn admin-btn-sm admin-btn-primary" style="margin-top: 0.5rem;">Resend</a>
                <?php endif; ?>
            </div>
        </div>
        <?php endforeach; ?>
        ...
        <div class="admin-detail-actions" style="margin-top: 1.5rem;">
            ...
            <a href="?action=mark_visited&id=<?= $detail_journey['id']; ?>&filter=<?= $filter; ?>" class="admin-btn admin-btn-sm admin-btn-primary">Mark as Visited</a>
            <a href="?action=cancel&id=<?= $detail_journey['id']; ?>&filter=<?= $filter; ?>" class="admin-btn admin-btn-sm admin-btn-secondary">Cancel Journey</a>
            <?php endif; ?>
            <a href="?action=delete&id=<?= $detail_journey['id']; ?>&filter=<?= $filter; ?>" class="admin-btn admin-btn-sm admin-btn-danger" data-confirm-delete>Delete</a>
        </div>
    </div>
</div>
<?php endif; ?>
            ...
            <?php foreach ($journeys as $journey): ?>
                ...
                <div class="admin-post-row">
                    ...
                    <div class="admin-post-actions">
                        <a href="?view=<?= $journey['id']; ?>&filter=<?= $filter; ?>" class="admin-btn admin-btn-sm admin-btn-primary">View</a>
                        <?php if ($journey['status'] === 'active'): ?>
                        <a href="?action=cancel&id=<?= $journey['id']; ?>&filter=<?= $filter; ?>" class="admin-btn admin-btn-sm admin-btn-secondary" title="Cancel Journey">Cancel</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endforeach; ?>
...
.email-sequence-card:last-child {
    margin-bottom: 0;
}

.email-sequence-header {
    display: flex;
