...
<div class="admin-dashboard-header" style="margin-bottom: 1rem;">
    <div class="admin-dashboard-greeting">
        ...
        <a href="/adminnew/reading-plans/edit" class="btn btn-sm btn-primary">+ New Plan</a>
    </div>
...
<div class="admin-card">
    <div class="admin-card-header">
        <div class="admin-filter-tabs" style="margin: 0;">
            <a href="/adminnew/reading-plans" class="admin-filter-tab <?= !$statusFilter ? 'active' : ''; ?>">All</a>
            <a href="/adminnew/reading-plans?status=published" class="admin-filter-tab <?= $statusFilter === 'published' ? 'active' : ''; ?>">Published</a>
            <a href="/adminnew/reading-plans?status=draft" class="admin-filter-tab <?= $statusFilter === 'draft' ? 'active' : ''; ?>">Drafts</a>
        </div>
        <?php if (!empty($categories)): ?>
        <div class="admin-filter-selects">
            <select data-navigate-on-change class="admin-select-sm">
                <option value="/adminnew/reading-plans" <?= empty($categoryFilter) ? 'selected' : ''; ?>>All Categories</option>
                <?php foreach ($categories as $cat): ?>
                    <option value="/adminnew/reading-plans?category=<?= urlencode($cat); ?>" <?= $categoryFilter === $cat ? 'selected' : ''; ?>>
    ...
    <?php if (empty($plans)): ?>
        <div class="admin-empty-state">
            ...
            <p>No reading plans yet. <a href="/adminnew/reading-plans/edit">Create one</a></p>
        </div>
    <?php else: ?>
            ...
            <?php foreach ($plans as $plan): ?>
                <div class="admin-post-row">
                    ...
                    <div class="admin-post-actions">
                        <a href="/adminnew/reading-plans/edit/<?= $plan['id']; ?>" class="btn btn-xs btn-outline">Edit</a>
