<?php
/**
 * Reusable Media Picker Component for New Admin
 *
...
<!-- Media Picker Modal -->
<div id="media-picker-modal" class="admin-media-picker-overlay">
    <div class="admin-media-picker-modal">
        <div class="admin-media-picker-header">
            <h3>Select Image</h3>
            <button type="button" id="media-picker-close-btn" class="admin-media-picker-close">&times;</button>
        </div>
        <div class="admin-media-picker-filters">
            <input type="text" id="media-search" placeholder="Search images..." class="admin-input">
            <div id="media-tags" class="admin-media-tag-filters"></div>
        </div>
        <div class="admin-media-picker-body">
            <div id="media-picker-loading" class="admin-media-picker-loading">Loading...</div>
            <div id="media-picker-grid" class="admin-media-picker-grid"></div>
            <div id="media-picker-empty" class="admin-media-picker-empty">
                No images found. <a href="/adminnew/media">Upload one</a>.
            </div>
            <div id="media-picker-pagination" class="admin-media-picker-pagination">
                <button type="button" id="media-load-more" class="admin-btn admin-btn-secondary">
                    Load More
                </button>
                <span id="media-count" class="admin-media-count"></span>
            </div>
        </div>
    </div>
</div>
...
.admin-media-picker-clear-hidden {
    display: none;
}
...
.admin-media-picker-close:hover {
    color: var(--admin-text);
}
.admin-media-picker-filters {
...
.admin-media-tag-btn:hover {
    border-color: var(--tag-color, var(--admin-accent));
}
.admin-media-tag-btn.active {
    background: var(--tag-color, var(--admin-accent));
    color: white;
    border-color: var(--tag-color, var(--admin-accent));
}
...
(function() {
    ...
    window.openMediaPicker = function(callback) {
        ...
        // Reset tag buttons
        document.querySelectorAll('.admin-media-tag-btn').forEach(btn => btn.classList.remove('active'));
        ...
        try {
            ...
            if (result.tags && mediaPickerState.page === 1) {
                ...
                tagsContainer.innerHTML = result.tags.map(tag =>
                    `<button type="button" class="admin-media-tag-btn" data-tag="${tag.slug}" style="--tag-color: ${tag.color}">${tag.name}</button>`
                ).join('');
            }
    ...
    window.renderMediaGrid = function() {
        ...
        mediaPickerState.allItems.forEach(item => {
            ...
            div.className = 'admin-media-picker-item';
    ...
    window.filterByTag = function(tag) {
        const buttons = document.querySelectorAll('.admin-media-tag-btn');
    ...
    document.getElementById('media-tags')?.addEventListener('click', function(e) {
        const tagBtn = e.target.closest('.admin-media-tag-btn');
        if (tagBtn) {
...
function createImagePickerField($fieldName, $currentValue = '', $label = 'Image', $helpText = '') {
    ...
    $html .= '<label class="admin-label">' . htmlspecialchars($label) . '</label>';
    $html .= '<div class="admin-image-picker-field">';
    $html .= '    <input type="hidden" name="' . htmlspecialchars($fieldName) . '" id="' . $fieldId . '" value="' . htmlspecialchars($currentValue) . '">';
    $html .= '    <div class="admin-image-preview-container">';

    if ($hasImage) {
        $html .= '        <img src="' . htmlspecialchars($currentValue) . '" id="' . $fieldId . '_preview" class="admin-image-preview">';
    } else {
        $html .= '        <img src="" id="' . $fieldId . '_preview" class="admin-image-preview admin-image-preview-hidden">';
        $html .= '        <div id="' . $fieldId . '_placeholder" class="admin-image-placeholder">No image</div>';
    }

    $html .= '    </div>';
    $html .= '    <div class="admin-image-picker-actions">';
    $html .= '        <button type="button" class="admin-btn admin-btn-sm admin-btn-secondary admin-media-picker-select-btn" data-field="' . $fieldId . '">Select Image</button>';
    $html .= '        <button type="button" class="admin-btn admin-btn-sm admin-btn-secondary admin-media-picker-clear-btn' . ($hasImage ? '' : ' admin-media-picker-clear-hidden') . '" data-field="' . $fieldId . '" id="' . $fieldId . '_clear">Clear</button>';
    $html .= '    </div>';
    $html .= '</div>';

    if ($helpText) {
        $html .= '<div class="admin-form-help">' . htmlspecialchars($helpText) . '</div>';
    }
...
window.setImageFieldValue = function(fieldId, url) {
    ...
    if (preview) {
        preview.src = url;
        preview.classList.remove('admin-image-preview-hidden');
    }
    if (placeholder) {
        placeholder.style.display = 'none';
    }
    if (clearBtn) {
        clearBtn.classList.remove('admin-media-picker-clear-hidden');
    }
};
...
window.clearImageField = function(fieldId) {
    ...
    if (preview) {
        preview.classList.add('admin-image-preview-hidden');
        preview.src = '';
    }
    if (placeholder) {
    ...
    if (clearBtn) {
        clearBtn.classList.add('admin-media-picker-clear-hidden');
    }
};

// Event delegation for select/clear buttons
document.addEventListener('click', function(e) {
    const selectBtn = e.target.closest('.admin-media-picker-select-btn');
    if (selectBtn) {
    ...
    const clearBtn = e.target.closest('.admin-media-picker-clear-btn');
    if (clearBtn) {
